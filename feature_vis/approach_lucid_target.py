...
def calc_loss(img, model, start_image, tv, l1, l2, c, target):
...
class DeepDream(tf.Module):
    ...
    def __call__(self, img, steps, step_size, tv, l1, l2, pad, c, target):
        print("Tracing")
        ...
        for n in tf.range(steps):
            with tf.GradientTape() as tape:
                ...
                tape.watch(img)
                loss = calc_loss(img, model=self.model, start_image=start_image, tv=tv, l1=l1, l2=l2, c=c, target= target)
...
# Main Loop
def run_deep_dream_simple(img, deepdream, target, steps=100, step_size=0.01, save_every=50,file_path = os.path.join("feature_vis","output_images"), file_name = str(int(time.time())), tv=0, l1=0, l2=0, pad=0, c=0,
                          na="",  show_img = True):
...
def main(argv):
    ...
    TARGET = [eval(con) for con in CLI.target]
    if len(TARGET) == 1: TARGET = TARGET * len(CLI.neurons)
    print(TARGET)
    if CLI.img[0] == "None":
